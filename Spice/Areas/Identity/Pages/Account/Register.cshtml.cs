    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var roles = new[] { SD.KitchenUser, SD.FrontDeskUser, SD.ManagerUser, SD.CustomerEndUser };

                    foreach (var r in roles)
                    {
                        if (!await this._roleManager.RoleExistsAsync(r))
                        {
                            await this._roleManager.CreateAsync(new IdentityRole(r));
                        }
                    }
